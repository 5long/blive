...
function parseComment(buf) {
  ...
  var actualBuf = buf.slice(4, pktLength)
    , remainingBuf = buf.slice(pktLength)
    , payload

  try {
    payload = JSON.parse(actualBuf.toString("utf8"))
  } catch (e) {
  ...
  if (payload.cmd === 'SEND_GIFT') {
    return [{
      ...
      giftType: payload.data.giftType,
      giftName: payload.data.giftName,
      giftCount: payload.data.num,
      nick: payload.data.uname,
    }].concat(parse(remainingBuf))
  }
  ...
  return [{
    ...
    isVip: payload.info[2][3] === '1',
    isAdmin: payload.info[2][2] === 1,
  }].concat(parse(remainingBuf))
}
